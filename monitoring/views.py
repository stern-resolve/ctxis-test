...
def logins_by_server(request):
	...
	# responsive feel as we will not have network issues to contend with after initial 
	# data fetch

	# Would probably use an ORM here, but rough SQL...
	# select s.server_name, s.server_ip, l.login_date, u.username, from logins l where l.date >= '2017-01-01' and l.date <='2018-04-30'
	# join server s on l.server_id = s.server_id
	# join user u on l.user_id = u.user_id
	
	# TODO: think about a fail-safe limit on the number of rows returned - say, 10k
